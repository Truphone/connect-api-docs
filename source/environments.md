...
The main environment. Should only be used for live traffic. Releases on this environment happen on a scheduled basis and will be communicated to partners. Accounts created in the production environment will be marked as test until the Partner and Truphone agree on a go live date.

## Pre-Production

Base URL: https://services.truphone.com/connect-api-preprod

The Pre-Production environment was created to replace the Staging environment. It has production grade support and throughput, but the data created in this environment will be marked as test data, and therefore will not be shared with the production environment. The main goal is to allow App developers to integrate with the next version of the API, as well as to execute connectivity tests. A few considerations to have when using Pre-Production:
...
- Pre-production will still use test sims as Staging does today and the current commercial agreements and rules around resource recycling will be kept
